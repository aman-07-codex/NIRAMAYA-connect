import { Phone, MapPin, Award, Star, Send, Loader2 } from "lucide-react";
...
const DonorCard = ({ donor, emergency }: Props) => {
  ...
  const handleSendReport = async () => {
    ...
    if (error) {
      ...
      toast.success(`Blood report sent to ${donor.name}'s dashboard alerts.`);
    }
  };
  ...
  >
    ...
    <div className="mt-4 flex flex-col gap-2">
      ...
      <button
        ...
        disabled={isSending}
        className="inline-flex w-full items-center justify-center gap-2 rounded-lg border border-primary text-primary px-4 py-2 text-sm font-semibold hover:bg-primary/10 transition-colors disabled:opacity-50"
      >
