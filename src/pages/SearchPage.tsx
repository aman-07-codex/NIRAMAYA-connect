    ...
    <div className="container py-8">
      ...
      <div className="mb-6 flex flex-wrap items-end gap-3 rounded-lg border bg-card p-4 shadow-sm animate-fade-in-up stagger-1">
        ...
        <button
          ...
          className="inline-flex flex-1 md:flex-none justify-center items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 transition-colors"
        >
          Search
        </button>
        <button
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in border-2 border-dashed rounded-lg bg-card/50">
          <Droplets className="h-12 w-12 text-muted-foreground/40" />
          <p className="mt-4 text-lg font-medium text-muted-foreground">Please perform a search to view donors</p>
          <p className="text-sm text-muted-foreground">Select criteria and click Search to submit a request</p>
        </div>
      ) : (
        <>
      ...
      {showReportModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-lg border bg-card p-6 shadow-lg animate-in fade-in zoom-in-95">
            <h3 className="text-lg font-bold mb-2">Submit requested blood report</h3>
            <p className="text-sm text-muted-foreground mb-4">
              Please upload the patient's blood report to proceed with the search.
            </p>
            <input 
              type="file" 
              className="w-full mb-4 text-sm file:mr-4 file:rounded-md file:border-0 file:bg-primary/10 file:px-4 file:py-2 file:text-sm file:font-semibold file:text-primary hover:file:bg-primary/20"
            />
            <div className="flex justify-end gap-2">
              <button 
                onClick={() => setShowReportModal(false)}
                className="rounded-md px-4 py-2 text-sm font-semibold hover:bg-muted"
              >
                Cancel
              </button>
              <button 
                onClick={() => {
                  toast.success("Blood report submitted! Searching donors...");
                  setHasSearched(true);
